...
更新策略：A>0 的答案概率↑、A<0 的↓（带 ratio / clip 稳定）
      ▼
采样级可验证正确率 0.50 → 1.00  + best-of-N 把成功率进一步滚高
```
...
- 采样级可验证正确率 **0.500 → 1.000**（严格上升）；
- 正确候选的概率从 **0.333 → 0.999**（机制上确实推高了 P(正确)）；
- 优势全程有限（`isfinite` 断言）；
- 退出码 0、打印 `[PASS]`。

**Step 5｜best-of-N / rejection sampling** — 在 SFT 弱策略上 `best_of_n_accuracy` 显示 `N=1 0.315 < N=4 0.795 < N=16 0.998`（N 越大成功率越高），并对近乎满分的 GRPO 策略打印对照（已饱和、提升幅度自然变小）。
