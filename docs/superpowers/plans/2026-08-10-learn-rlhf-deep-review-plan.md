...
- [ ] 统一 `versions.md`、AGENTS、根表、m00、m07、m08、m12 的 v0.6/v0.7 语义。
- [ ] 修复 m12 的最终策略身份、Best-of-N 指标和稳定失败。
- [ ] 清除 m12 重复前向/死变量，并把 KL 符号检查改为代数恒等式。
- [ ] 用静态检查和 m07/m08/m12 实跑验证。
...
- [ ] 让 m12 的每个阶段名、实际 reward 来源、policy 身份和总结指标严格一致。
- [ ] 明确经典 RM→PPO 与 verifier/RLVR 的边界；若两者都展示，分别命名并验证。
- [ ] 修复共享 value head 的优化器所有权。
- [ ] 更新根 README：学习路径、运行/验收命令、教学边界、模块能力矩阵。
- [ ] 对所有 README 做模板、版本、命令、指标、前后序链接审计。
...
- [ ] AST 全量通过。
- [ ] m01–m12 全量从仓库根目录运行，全部退出 0 且打印 `[PASS]`。
- [ ] 静态扫描无本计划列出的死变量、重复更新和版本漂移。
- [ ] `git diff --check` 通过；ignored checkpoint 不进入索引；工作区只含预期变更。
- [ ] 按设计“不变量”逐项给出当前文件/命令证据。
- [ ] 仅在所有证据齐全后结束目标。
